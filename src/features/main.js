...
$('#menu-item-linkto-storage-center-vm').on('click', function(){
    ...
    cockpit.spawn(["python3", pluginpath+"/python/url/create_address.py", "storageCenterVm"])
        ...
        .catch(function(data){
            createLoggerInfo(":::menu-item-linkto-storage-center-vm click Error :::");
    ...
    return new Promise((resolve) => {
        ...
        .then(function(data){            
            ...
            if(retVal.val.bootstrap.scvm == "false"){ //bootstrap.sh 실행 전
                ...
                $("#scvm-after-bootstrap-separator").remove();
                $("#scvm-after-bootstrap-run").remove();
                $("#scvm-before-bootstrap-run").remove();
    ...
    if(step1!="true"){
        ...
        if(step2=="HEALTH_ERR"||step2==null){
            ...
            if(step3!="true"){
                ...
                if(step7!="true" && step4=="HEALTH_ERR"||step4==null){
                    // 스토리지센터 연결 버튼 show
                    ...
                    if(step7!="true" && step5=="HEALTH_ERR1"||step5=="HEALTH_ERR2"||step5==null){
                        //클라우드센터 VM 배포 버튼, 스토리지센터 연결 버튼 show
                        $('#button-open-modal-wizard-cloud-vm').show()
                        $('#button-link-storage-center-dashboard').show()
                        if(step7!="true" && step5=="HEALTH_ERR1"||step5==null){
                        ...
                        if(step7!="true" && step6=="HEALTH_ERR"||step6==null){
                            //클라우드센터 VM 배포 버튼, 스토리지센터 연결 버튼 show
                            $('#button-open-modal-wizard-cloud-vm').show()
                            $('#button-link-storage-center-dashboard').show()
