...
$('#button-auto-shutdown').on('click', async function(){
    ...
    $('#div-modal-spinner-header-txt').text('NFS Mount 여부를 체크를 하고 있습니다.');
    createLoggerInfo("cloud-nfs-status mountCheck");
    cockpit.spawn(['/usr/bin/python3', '/root/test.py', 'check_nfs'])
    .then(function(data){
        var retVal = JSON.parse(data);
        if(retVal.code == 200){
            console.log("nfs 체크 여부 성공"); 
            // 7.	클라우드센터 가상머신 정지
            $('#dropdown-menu-cloud-cluster-status').toggle();
                ...
                if(retVal.code == 200){
                    ...
                    .then(function(data){            
                        ...
                        if(retVal.code == "200"){
                            ...
                            // cockpit.spawn(["python3", pluginpath+"/python/scc_status/test.py","test"])
                            cockpit.spawn(["/usr/bin/python3", "/root/test.py","test"])
                            .then(function(data){
                                var retVal = JSON.parse(data);
                                if(retVal.code == "200"){
                                    console.log(data);
                                    ...
                                    cockpit.spawn(["/usr/bin/python3", "/root/test.py","shutdown"])
                                    .then(function(data){  
                                        console.log(data);
                                        location.reload();
