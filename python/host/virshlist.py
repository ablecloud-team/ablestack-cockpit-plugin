...
for vm in vms:
    if (vm['Name'] == 'ccvm'):
        ...
        ret = ssh('-o', 'StrictHostKeyChecking=no', 'ccvm-mngt', '/usr/bin/df', '-h').splitlines()
        ret.reverse()
        vm['blk'] = ret
        for line in ret[:]:
            if 'root' in line:
        ...
        try:
            ret = ssh('-o', 'StrictHostKeyChecking=no', 'ccvm-mngt', '/usr/sbin/route', '-n', '|', 'grep', '-P', '"^0.0.0.0|UG"').splitlines()
            for line in ret[:]:
                items = line.split()
                vm['GW'] = items[1]
        ...
        # DNS 정보 확인
        try:
            ret = ssh('-o', 'StrictHostKeyChecking=no', 'ccvm-mngt', '/usr/bin/cat', '-n', '/etc/resolv.conf', '|', 'awk', "'$1 == 2 {print $3}'").splitlines()
            for line in ret[:]:
                items = line.split()
                vm['DNS'] = items[0]
        except Exception as e:
        ...
        try :
            vm['MOLD_SERVICE_STATUE'] = ssh('-o', 'StrictHostKeyChecking=no', 'ccvm-mngt', 'systemctl is-active cloudstack-management.service').splitlines()
        except Exception as e:
            vm['MOLD_SERVICE_STATUE'] = 'inactive'.splitlines()
        try :
            vm['MOLD_DB_STATUE'] = ssh('-o', 'StrictHostKeyChecking=no', 'ccvm-mngt', 'systemctl is-active mysqld').splitlines()
        except Exception as e:
